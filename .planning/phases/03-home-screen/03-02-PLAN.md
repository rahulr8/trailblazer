...
- Do NOT use `pagingEnabled` — use `snapToInterval` with `decelerationRate="fast"` for consistent cross-platform behavior with custom card widths
...
- Top-right corner: `RefreshCw` icon from lucide-react-native (size 20, color white, opacity 0.8) inside a small `Pressable`. On press: call `onRefreshMotivation` callback if provided (HeroSwiper does NOT maintain its own affirmation state — the parent Home screen manages affirmation cycling via pull-to-refresh and passes the current text as `motivationText` prop)
...
**Important:** `snapToInterval` is already specified above — do NOT add `pagingEnabled` as they conflict when used together on custom card widths.
  </action>
  <verify>
